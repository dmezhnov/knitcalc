...
/// Canonical launcher, generated by tool/packaging_metadata.dart from
/// packaging/metadata/metadata.yaml; `@EXEC@` is the only thing filled in here.
const String desktopTemplate = 'packaging/desktop/$appId.desktop';
...
/// `.desktop` entry text, read from the canonical launcher that
/// tool/packaging_metadata.dart renders from packaging/metadata/metadata.yaml —
/// the same names and comments every other channel ships. Wayland matches the
/// window to its icon purely by the file basename (== app_id) and the `Icon=`
/// key, so [exec] is only what the menu entry launches: `@EXEC@` (filled in by
/// install.sh) for the bundle, an absolute path for the dev install.
String _desktopEntry(String exec) {
  final template = File(desktopTemplate);
  if (!template.existsSync()) {
    stderr.writeln(
      'linux_desktop_install: $desktopTemplate not found '
      '(run `mise metadata` first).',
    );
    exit(1);
  }
  return template.readAsStringSync().replaceAll('@EXEC@', exec);
}
